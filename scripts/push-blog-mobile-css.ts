...
async function pushMobileCSS() {
  const owner = 'rupeshtiwari';
  const repo = 'blogs';
  const branch = 'master';
  
  console.log('📱 Pushing mobile CSS to blog...\n');
  
  const accessToken = await getAccessToken();
  const octokit = new Octokit({ auth: accessToken });
  
  try {
    console.log('1️⃣ Pushing mobile-responsive.css...');
    await pushFile(
      octokit, owner, repo, branch,
      'assets/css/mobile-responsive.css',
      './blog-content/assets/css/mobile-responsive.css',
      'Add mobile responsiveness CSS for Minimal Mistakes theme'
    );
    
    console.log('\n2️⃣ Updating head.html with CSS link...');
    await pushFile(
      octokit, owner, repo, branch,
      '_includes/head.html',
      './blog-content/_includes/head.html',
      'Link mobile responsive CSS in head'
    );
    
    console.log('\n✅ Mobile CSS pushed successfully!');
...
pushMobileCSS().catch(console.error);
